...
fprintf('Working on product: %s\n', [product, ' ', num2str(class_lo(class)), '-', num2str(class_hi(class))]);

fid = fopen(['../outputs/imperviousness_analysis/hs18_IManalysis_IM_', num2str(class_lo(class)), '-', num2str(class_hi(class)), '_noShutDown_regional_and_seasonal_All_weeks_contingency_stats_', product, 'csv'], 'w');
...
for per_i = 1:numel(dates_i)
    ...
    %**Exclude shutdown weeks: PRECIPRATE.EVAP.20181226.161000.gz - PRECIPRATE.EVAP.20190206.155600.gz
    st_shut = datenum('20181226.161000', 'yyyymmdd.HHMMSS');
    end_shut = datenum('20190206.155600', 'yyyymmdd.HHMMSS');
    if ((period(1) < st_shut && period(end) > st_shut) || (period(1) >= st_shut && period(1) <= end_shut))
	fprintf('Skipping period: %s - %s due to shutdown\n', datestr(period(1), 'yyyymmddHHMM'), datestr(period(end), 'yyyymmddHHMM'));
	continue;
    end

    fprintf('Working on period: %s - %s\n', datestr(period(1), 'yyyymmddHHMM'), datestr(period(end), 'yyyymmddHHMM'));

    load(['../outputs/imperviousness_analysis/', product_folder, 'CT_stats_IM_', num2str(class_lo(class)), '-', num2str(class_hi(class)), '_MultiThreshold_Run', outFile_product, '_', datestr(period(1), 'yyyymmddHHMM'), '-', datestr(period(end), 'yyyymmddHHMM'), '.mat']);

    fprintf('%s\n', ['CT_stats_IM_', num2str(class_lo(class)), '-', num2str(class_hi(class)), '_MultiThreshold_Run', outFile_product, '_', datestr(period(1), 'yyyymmddHHMM'), '-', datestr(period(end), 'yyyymmddHHMM'), '.mat']);
